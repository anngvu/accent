...
(defn prep-dataset-manifest-handler
  ...
  (let [prod "https://raw.githubusercontent.com/Sage-Bionetworks/data_curator_config/refs/heads/prod/"
    ...
    {:messages [{:role "user"
              ...
              {:role "assistant"
               :content {:type "text"
                         :text (str "The first step is to generate a metadata manifest for `folder_id` " folder_id "(assuming the id given is indeed a qualifying Synapse folder)." 
                                    "I will refer to the DCC-specific configuration for parameters like `asset_view`. "
                                    "To infer `data_type` parameter for this dataset, I will check the data types/templates defined in the DCC data model. " 
                                    "Then, looking at the dataset, if its type is not clear, my protocol suggests that I try to confirm the type by interviewing you about the data or sampling some files (with your permission and if I have the tools available). "
