...
" Step 5: Identify Template - Confirm which GC data submission template this sheet’s data belongs to, based on the mapping. In most cases, the sheet maps to a single GDC entity template (for example, "Case", "Sample", "Clinical", etc.). If the sheet’s data spans multiple GC templates, handle one template at a time or split the data as appropriate."
...
" Step 7: Write Output - Use the write_csv function to output the transformed data in the structure of the identified GDC template. Name or label the output clearly (e.g., using the GDC template name) so it’s distinguishable. The output should be a CSV (or TSV as required by GC) with columns matching the GC template’s expected fields and rows representing each record from the input sheet."
